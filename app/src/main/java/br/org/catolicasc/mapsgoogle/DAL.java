...
class DAL {
    ...
    boolean update(int id, String title, String author, String publisher) {
        ...
        // Par de nomes de colunas + valores, para atualização no banco
        values = new ContentValues();
        values.put(CreateDatabase.NOME, title);
        values.put(CreateDatabase.EMAIL, author);
        values.put(CreateDatabase.LATITUDE, publisher);
...
//        cursor = db.rawQuery("SELECT * FROM contato WHERE _id = ?", new String[]{"1"});
